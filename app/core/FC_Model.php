...
class FC_Model extends CI_Model{
	...
	public function _viewproducts($param = '', $catalogue = '', $lang = 'vietnamese'){
		$adults = $this->input->get('adults');
		$children = $this->input->get('children');
		$title = $this->input->get('title');
		if($title > 0){
			$wheret = '`pr`.`id` = \''.$title.'\' AND';
		}else{
			$wheret = '';
		}
		if($adults > 0){
			$wherea = '`pr`.`adults` = \''.$adults.'\' AND';
		}else{
			$wherea = '';
		}
		if($children > 0){
			$wherec = '`pr`.`children` = \''.$children.'\' AND';
		}else{
			$wherec = '';
		}
		$param['select'] = (isset($param['select'])) ? $param['select'] : '';
		...
		$data = $this->db->query('
			...
			FROM `catalogues_relationship` as `att`
			INNER JOIN `'.$param['modules'].'` as `pr`
			WHERE `pr`.`trash` = 0 AND  '.$wheret.' '.$wherea.' '.$wherec.' `pr`.`alanguage` = \''.$lang.'\' AND `pr`.`publish` = 1 AND `att`.`modules` = \''.$param['modules'].'\' AND `att`.`modulesid` = `pr`.`id`'.$attribute['query'].'
 			GROUP BY `att`.`modulesid`
			ORDER BY '.((!empty($param['order_by'])) ? $param['order_by'] : '`pr`.`order` asc, `pr`.`id` desc').'
			LIMIT '.($param['start']).', '.$param['limit'].'
		')->result_array();
