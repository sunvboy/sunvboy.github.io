...
class BackendAddress_Model extends CI_Model
{
    ...
    public function create($user = '', $albums = '')
    {
        $data = array(
...
//			'typeoff' =>  $this->input->post('typeoff'),
    ...
    public function update($id = 0, $user = '', $albums = '')
    {
        $data = array(
            'title' => $this->input->post('title'),
            'slug' => slug($this->input->post('title')),
    ...
    public function countall($typeoff = 0)
    {
        $this->db->where(array('trash' => 0, 'typeoff' => $typeoff));
    ...
    public function view($start = 0, $limit = 0, $typeoff = 0)
    {
        $this->db->where(array('trash' => 0, 'typeoff' => $typeoff));
        $keyword = $this->input->get('keyword');
        if (!empty($keyword)) {
    ...
    {
        $this->db->where(array('address.trash' => 0));
    ...
    public function UpdateBatchByField($data = NULL, $field = 'id'){
        $result = $this->db->update_batch('address', $data, $field);
        $this->db->flush_cache();
        return $result;
    }
}