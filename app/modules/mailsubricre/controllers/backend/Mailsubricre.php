...
class Mailsubricre extends FC_Controller
{
    ...
    public function viewctv($page = 1)
    {
        $page = (int)$page;
        $config['total_rows'] = $this->Backendmailsubricre_Model->countallctv();
        if ($config['total_rows'] > 0) {
            $this->load->library('pagination');
            $config['base_url'] = base_url('mailsubricre/backend/mailsubricre/view');
            $config['suffix'] = $this->config->item('url_suffix') . (!empty($_SERVER['QUERY_STRING']) ? ('?' . $_SERVER['QUERY_STRING']) : '');
            ...
            $page = ($page > $totalPage) ? $totalPage : $page;
            $page = $page - 1;
            $data['listemail'] = $this->Backendmailsubricre_Model->viewctv(($page * $config['per_page']), $config['per_page']);
        }
        // echo $this->db->last_query();
        $data['template'] = 'mailsubricre/backend/mailsubricre/viewctv';
        $this->load->view('dashboard/backend/layouts/home', isset($data) ? $data : NULL);
    }
    public function viewguiyeucau($page = 1)
    {
        $page = (int)$page;
        $config['total_rows'] = $this->Backendmailsubricre_Model->countallguiyeucau();
        if ($config['total_rows'] > 0) {
            $this->load->library('pagination');
            $config['base_url'] = base_url('mailsubricre/backend/mailsubricre/viewguiyeucau');
            $config['suffix'] = $this->config->item('url_suffix') . (!empty($_SERVER['QUERY_STRING']) ? ('?' . $_SERVER['QUERY_STRING']) : '');
            $config['first_url'] = $config['base_url'] . $config['suffix'];
            $config['per_page'] = 10;
            $config['uri_segment'] = 5;
            ...
            $page = ($page > $totalPage) ? $totalPage : $page;
            $page = $page - 1;
            $data['listemail'] = $this->Backendmailsubricre_Model->viewguiyeucau(($page * $config['per_page']), $config['per_page']);
        }
        // echo $this->db->last_query();
        $data['template'] = 'mailsubricre/backend/mailsubricre/viewguiyeucau';
        $this->load->view('dashboard/backend/layouts/home', isset($data) ? $data : NULL);
    }
    ...
    public function read($id = 0)
    {
        ...
        $data['payment_list'] = $this->Backendmailsubricre_Model->ReadByConditionDH(array(
            'select' => 'id, name, quanty, note',
            'table' => 'product_items',
            'where' => array('mail_id' => $id),
            'limit' => 10,
            'order_by' => 'id desc',
        ));
        //echo "<pre>";var_dump($data['payment_list']);die();
        ...
        if (!isset($data['Detailmailsubricre']) && !is_array($data['Detailmailsubricre']) && count($data['Detailmailsubricre']) == 0) {
            ...
            redirect_custom('mailsubricre/backend/mailsubricre/viewguiyeucau');
        }
    ...
    public function delete1($id = 0)
    {
        ...
        if ($this->input->post('delete')) {
            ...
            if ($flag > 0) {
                $this->session->set_flashdata('message-success', 'Xóa  thành công');
                redirect('mailsubricre/backend/mailsubricre/viewguiyeucau');
            }
        }
        $data['template'] = 'mailsubricre/backend/mailsubricre/delete';
        $this->load->view('dashboard/backend/layouts/home', isset($data) ? $data : NULL);
    ...
    public function delete2($id = 0)
    {
        ...
        if (!isset($data['Detailmailsubricre']) && !is_array($data['Detailmailsubricre']) && count($data['Detailmailsubricre']) == 0) {
            $this->session->set_flashdata('message-danger', ' không tồn tại');
            redirect_custom('mailsubricre/backend/mailsubricre/viewctv');
        }
        if ($this->input->post('delete')) {
            $flag = $this->Backendmailsubricre_Model->delete($id);
            if ($flag > 0) {
                $this->session->set_flashdata('message-success', 'Xóa  thành công');
                redirect('mailsubricre/backend/mailsubricre/viewctv');
            }
        }
        $data['template'] = 'mailsubricre/backend/mailsubricre/delete';
        $this->load->view('dashboard/backend/layouts/home', isset($data) ? $data : NULL);
