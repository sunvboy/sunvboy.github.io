...
class Catalogues extends FC_Controller{
	...
	public function set($type = NULL, $id = 0){
		...
		$id = (int)$id;
		$data['supports'] = $this->BackendSupportsCatalogues_Model->ReadByField('id', $id, $this->fclang);
		$temp[$type] = (($data['supports'][$type] == 1)?0:1);
		$temp['userid_updated'] = $this->fcUser['id'];
		$temp['updated'] = gmdate('Y-m-d H:i:s', time() + 7*3600);
		$this->db->where('id', $id);
		$this->db->update('supports_catalogues', $temp);
		 //echo $this->db->last_query();die;
		redirect((!empty($redirect)) ? $redirect : 'supports/backend/catalogues/view');
	}
