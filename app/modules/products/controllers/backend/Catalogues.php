	...
	public function Create(){
		...
		if($this->input->post('create')){
			...
			if ($this->form_validation->run($this)){
				...
				$resultid = $this->BackendProductsCatalogues_Model->Create($this->fcUser, $album_data, $this->fclang);
				if($resultid > 0){
	...
	public function Update($id = 0){
		...
		if($this->input->post('update')){
			...
			if ($this->form_validation->run($this)){
				...
				$flag = $this->BackendProductsCatalogues_Model->UpdateByPost(array('id'=> $id, 'user' => $this->fcUser), $album_data);
				if($flag > 0){
					$canonical = slug($this->input->post('canonical'));
					if(!empty($canonical)){
