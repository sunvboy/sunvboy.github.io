...
class FrontendProducts_Model extends FC_Model{
	...
	public function getCountry($page){
		$offset = 10*$page;
		$limit = 10;
		$sql = "select * from products_catalogues limit $offset ,$limit";
		$result = $this->db->query($sql)->result();
		//echo $this->db->last_query();die;
		return $result;
	}

//	public function getCountry($param = '',$page){
//		$offset = 2*$page;
//		$limit = 2;
...
//	}
	...
	public function ReadAllSP($start = 0, $limit = 0){
	...
	public function ReadAllNews($start = 0, $limit = 0){
		$this->db->where(array('products.trash' => 0));
		$this->db->select('products.*');
		$this->db->from('products');
...
// Lấy thuộc tính của sản phẩm
	...
	public function AttributesAllTheTime($productsid  = 0){
 	...
 	public function Attributes($productsid = 0, $catid = 0){
		$this->db->select('attributes.id, attributes.title, attributes.slug, attributes.canonical,attributes.cataloguesid');
	...
	public function countajaxproduct($attribute = '', $cataloguesid = 0, $lang = 'vietnamese'){
 	...
 	public function viewajaxproduct($start = 0, $limit = 0, $attribute = '', $cataloguesid = 0, $lang = 'vietnamese'){
	  	...
	  	$relationship['query'] = (!empty($relationship['query'])?' AND ('.$relationship['query'].')':'');
	  	$count = $this->db->query('
			SELECT `pr`.`id`, `pr`.`title`, `pr`.`slug`, `pr`.`canonical`, `pr`.`images`, `pr`.`price`, `pr`.`saleoff`,`pr`.`highlight`,  `att`.`productsid`, COUNT(`productsid`) as `total`
			FROM `attributes_relationship` as `att`
			INNER JOIN `products` as `pr` ON `pr`.`id` = `att`.`productsid`
			INNER JOIN `catalogues_relationship` as `rls` ON `pr`.`cataloguesid`
	...
	public function ReadByFieldAddress($id){
		$this->db->from('province');
	...
	public function viewaddress($title,$id){
		$this->db->select('*');
		$this->db->where(array('trash' => 0, 'publish' => 1,$title=>$id));
		$this->db->from('places');
