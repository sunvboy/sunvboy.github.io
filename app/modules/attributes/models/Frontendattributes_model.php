...
class FrontendAttributes_Model extends CI_Model{
	...
	//filter
	public function CountAllFilter($catalogues = '', $attrid = 0){
		$attribute['query'] = '';
		$attribute['total'] = 0;
		if($attrid > 0){
			$attribute['query'] = $attribute['query'].(empty($attribute['query'])?('`att`.`attrid` = '.$attrid):(' OR `att`.`attrid` = '.$attrid));
			$attribute['total'] = $attribute['total'] + 1;
		}
	...
	public function ReadAllFilter($catalogues = '', $attrid = 0, $start = 0, $limit = 10){
		$attribute['query'] = '';
		$attribute['total'] = 0;
		if($attrid > 0){
			$attribute['query'] = $attribute['query'].(empty($attribute['query'])?('`att`.`attrid` = '.$attrid):(' OR `att`.`attrid` = '.$attrid));
		...
		$attribute['having'] = (($attribute['total'] > 0)?(' HAVING `total` = '.$attribute['total']):'');
		$data = $this->db->query('
			SELECT `pr`.`psale`,`pr`.`cataloguesid`,`pr`.`id`, `pr`.`title`, `pr`.`slug`, `pr`.`canonical`, `pr`.`images`, `pr`.`price`, `pr`.`saleoff`, `pr`.`code`, `att`.`productsid`, COUNT(`productsid`) as `total`
			FROM `attributes_relationship` as `att`
			INNER JOIN `products` as `pr`
			WHERE `pr`.`trash` = 0 AND `pr`.`publish` = 1 AND `att`.`productsid` = `pr`.`id`'.$attribute['query'].(!empty($str_where) ? ' AND '.$str_where:'').'
 			GROUP BY `att`.`productsid`'.$attribute['having'].'
