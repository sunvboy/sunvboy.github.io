...
//			$this->form_validation->set_rules('content', 'Nội dung', 'trim|required');
			if ($this->form_validation->run($this)){
				...
				if(isset($album_data) && is_array($album_data)  && count($album_data) && isset($album['content']) && is_array($album['content']) && count($album['content']) && isset($album['title']) && is_array($album['title']) && count($album['title']) && isset($album['description']) && is_array($album['description']) && count($album['description'])) {
					foreach ($album_data as $key => $val) {
						$album_data[$key]['title'] = $album['title'][$key];
						$album_data[$key]['description'] = $album['description'][$key];
						$album_data[$key]['content'] = $album['content'][$key];
					}
				}
				...
				if(isset($album_data_2) && is_array($album_data_2)  && count($album_data_2) && isset($albumlist['content']) && is_array($albumlist['content']) && count($albumlist['content']) && isset($albumlist['title']) && is_array($albumlist['title']) && count($albumlist['title']) && isset($albumlist['description']) && is_array($albumlist['description']) && count($albumlist['description'])) {
					foreach ($album_data_2 as $key => $val) {
						...
						$album_data_2[$key]['content'] = $albumlist['content'][$key];
					}
				}


				$resultid = $this->BackendArticles_Model->Create($this->fcUser, $data['catalogue'], $album_data,$album_data_2, $this->fclang);
				if($resultid > 0){
...
//			$this->form_validation->set_rules('content', 'Nội dung', 'trim|required');
			if ($this->form_validation->run($this)){
				...
				$flag = $this->BackendArticles_Model->UpdateByPost('id', $id, $this->fcUser, $catalogues, $album_data,$album_data_2);
				if($flag > 0){
